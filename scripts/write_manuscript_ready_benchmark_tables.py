...
def main() -> None:
    ...
    cfmid_complete_query_subset = read_csv(
        ROOT
        / "results"
        / "casmi2022_cfmid_native_precomputed_complete_query_subset_v1"
        / "casmi2022_cfmid_native_precomputed_complete_query_subset_summary.csv"
    )
    ...
    complete_query_hybrid_subset = read_csv(
        ROOT
        / "results"
        / "casmi2022_cfmid_ms2deepscore_complete_query_hybrid_subset_v1"
        / "casmi2022_cfmid_ms2deepscore_complete_query_hybrid_subset_summary.csv"
    )
    ...
    if not cfmid_complete_query_subset.empty:
        complete_subset = cfmid_complete_query_subset.rename(columns={"claim_guardrail": "notes"})
        complete_subset["dataset"] = "CASMI2022"
        complete_subset["model"] = "CFM-ID complete-query subset"
        casmi = pd.concat([casmi, keep_existing(complete_subset, metric_cols)], ignore_index=True)
    ...
    if not complete_query_hybrid_subset.empty:
        hybrid_complete = complete_query_hybrid_subset.rename(columns={"claim_guardrail": "notes"})
        hybrid_complete["dataset"] = "CASMI2022"
        hybrid_complete["model"] = "CFM-ID + MS2DeepScore complete-query hybrid subset"
        casmi = pd.concat([casmi, keep_existing(hybrid_complete, metric_cols)], ignore_index=True)
        ...
        [
            {
                ...
                "complete_query_subset_status": "completed_full_candidate_set_for_selected_query"
                if not cfmid_complete_query_subset.empty
                else "",
                "complete_query_expansion_status": (
                    f"status={cfmid_complete_query_expansion.get('status')} "
                    f"query={cfmid_complete_query_expansion.get('query_id')} "
                    f"predicted={cfmid_complete_query_expansion.get('predicted_spectrum_ids')}/"
                    ...
                    if cfmid_complete_query_expansion
                    else ""
                ),
                "full_run_manifest": (
            ...
            },
            {
                ...
                "subset_status": (
                    "pretrained_model_and_cpu_env_verified; candidate-limited hybrid subset and complete-query "
                    "hybrid subset available; complete full-CASMI candidate spectrum library missing"
                ),
    ...
    report = [
        ...
        "- The CFM-ID complete-query subset row uses the full candidate set for selected low-candidate queries, but it is not a full CASMI CFM-ID baseline.",
        "- Query 35 is recorded as completed complete-query subset evidence after all 233 candidate spectra were generated and ranked; it is included in the subset row, not reported as full CASMI.",
        "- The CFM-ID + MS2DeepScore row is a generated-spectrum hybrid subset, not native MS2DeepScore and not a full CASMI benchmark.",
        "- The CFM-ID + MS2DeepScore complete-query hybrid row uses the full candidate set for selected low-candidate queries, but candidate spectra are generated by CFM-ID; it is not native MS2DeepScore and not a full CASMI benchmark.",
    ...
    write_json(
        OUTDIR / "audit_summary.json",
        {
            ...
            "cfmid_complete_query_expansion_status": cfmid_complete_query_expansion.get("status"),
            "ms2deepscore_full_candidate_ranking_available": False,
            "ms2deepscore_complete_query_hybrid_subset_available": not complete_query_hybrid_subset.empty,
